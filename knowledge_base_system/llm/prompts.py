...
- `heading_level`: 仅 title 元素，标题层级 1~4（对应 h1~h4）
- `language`: 仅 code 元素，编程语言标识（英文值，如 python / go / java / cpp / rust / sql / json / yaml / shell / bash 等）
- `structured_data`: 结构化附加数据（table 的表格数据、list 的嵌套结构等）
- `asset_ids` / `asset_descriptions`: 关联的图片/视频资源及其多模态描述
...
### table — 表格元素
- structured_data.table 包含 headers（表头列表）、rows（行数组，每行含 cells）
- 处理方式：将表格转为自然语言段落——先说明表格用途，再逐行转述
...
4. 关联图片/视频资源的元素，将其 asset_descriptions 自然融入 content
...
- 若元素附带 asset_descriptions（多模态模型描述），必须将该描述自然融合到 chunk 正文中
- 若资源没有 asset_descriptions，仅根据元素的 text（caption/alt）处理，不编造
- 资源链接本身是有价值的参考信息——将资源周围的文字说明融入正文，用 asset_refs 引用
