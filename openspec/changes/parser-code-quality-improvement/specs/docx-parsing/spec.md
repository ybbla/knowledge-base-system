...
- **GIVEN** DOCX 文档使用非英文 Word 版本创建，标题样式名为 "标题 1"、"Titre 1" 或 "Überschrift 1"
- **WHEN** 解析该 DOCX 文档
- **THEN** 解析器 SHALL 通过样式类型（`WD_STYLE_TYPE.PARAGRAPH`）和样式名中的标题关键词（含中文"标题"、法文"Titre"、德文"Überschrift"等）综合判断标题层级
- **AND** 非英文标题正确生成 `title` 类型 ParsedElement
...
- **GIVEN** DOCX 文档以 `metadata.raw_content` 形式提供原始字节
- **WHEN** 调用 `DocxParser.parse(doc)` 成功返回
- **THEN** `result.doc.metadata` 中 SHALL 不再包含 `"raw_content"` 键

### Requirement: DOCX 解析器实现统一解析器接口

系统 SHALL 实现 `DocumentParser` 抽象接口，声明 `SUPPORTED_TYPES = {"docx"}` 和 `RAW_CONTENT_FORMAT = "binary"`。
