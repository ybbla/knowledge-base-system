# Eval Dataset Auto-generation（Delta）

## REMOVED Requirements

### Requirement: 命令行手动生成评测数据

**Reason**: 手动 CLI 生成 (`gen_dataset.py --category --count --dry-run` 等) 与入库自动生成功能重复。用户可直接在入库后修改 LLM 生成的评测数据，无需手动触发生成新数据。
**Migration**: 所有评测数据生成由入库自动触发完成。如需批量生成，通过重新入库文档触发。
