...
系统 SHALL 根据文档的 `source_type` 通过解析器注册表自动选择对应解析器，将文档解析为 ParsedElement 树，保留文档结构。当前支持的格式包括 Markdown/TXT、DOCX、XLSX 和 HTML/HTM。
...
- **WHEN** Markdown 或 HTML 文档包含图片链接
- **THEN** 解析器生成或关联 `image` 资源，`asset_ids` 引用创建的 Asset，`text` 包含可用的 alt 文本或图片标记

#### Scenario: 解析视频链接

- **WHEN** Markdown、XLSX 或 HTML 文档包含视频 URL 或 `[video](https://example.com/video.mp4)`
