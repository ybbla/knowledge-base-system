...
#### Scenario: 注册 MarkdownParser、DocxParser、XlsxParser 和 HtmlParser

- **WHEN** 向注册表注册 `MarkdownParser`（`SUPPORTED_TYPES={"markdown", "md", "txt", "text"}`）、`DocxParser`（`SUPPORTED_TYPES={"docx"}`）、`XlsxParser`（`SUPPORTED_TYPES={"xlsx"}`）和 `HtmlParser`（`SUPPORTED_TYPES={"html", "htm"}`）
...
- **AND** `registry.get("MD")`、`registry.get("XLSX")` 和 `registry.get("HTML")` 大小写不敏感返回对应解析器实例
