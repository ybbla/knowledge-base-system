...
**Reason**: PATCH 端点和乐观锁机制整体移除。文档更新改为"上传新版本替换"的模式。
**Migration**: 通过 `POST /api/v1/documents/upload` + `replace_doc_id` + `confirm_replace=true` 实现文档更新。

### Requirement: 文档可触发入库动作
**Reason**: 不再需要单独的重新处理接口。用户可通过重新上传文件来达到重新入库的效果。
**Migration**: 使用 `POST /api/v1/documents/upload` 上传文件触发入库。
