...
提供文档更新流程，包括上传时自动检测同名文件、用户确认更新、软删除旧版本并创建新版本的完整交互体验。更新流程不实现增量合并，而是"删除旧 → 创建新"的全量替换模式。

## MODIFIED Requirements

> **注意**：此 spec 在 `simplify-doc-upload-flow` 变更中首次引入（ADDED），本变更在此基础上进一步清理 PATCH 端点和增量更新模式，统一为"软删除旧文档 + 创建新文档"的全量替换模式。基础的同名文件检测、确认替换、更新按钮逻辑已在 `simplify-doc-upload-flow` 中实现。

### Requirement: 上传时自动检测同名文件

系统 SHALL 在用户上传文件时，先检测 `source_hash` 是否重复。若不重复，再检测是否存在同名的活跃文档，并在返回结果中标注。
...
- **AND** 系统软删除旧文档的所有活跃知识块（`status=deleted`）
- **AND** 系统从索引中移除旧知识块
...
文档管理列表 SHALL 为每个活跃文档提供"更新"按钮，点击后进入更新流程。
