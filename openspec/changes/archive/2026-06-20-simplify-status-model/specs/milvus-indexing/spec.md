...
### Requirement: 批量更新知识块状态
**Reason**: `superseded` 状态移除后，不再需要通过 `update_status_batch(chunk_ids, "superseded")` 批量标记旧知识块。删除操作直接调用 `delete()` 移除 entity。
**Migration**: 文档更新时旧知识块直接通过 `delete()` 从索引中移除。
