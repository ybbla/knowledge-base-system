...
**Reason**: ChunkIndexStatus 移除后，不再需要独立的 reindex 端点。知识块索引通过创建和恢复操作自动触发。
**Migration**: 使用 `POST /api/v1/chunks/{chunk_id}/restore` 恢复知识块并自动触发重新索引。
