...
将文件上传和 Asset 存储从本地磁盘迁移至 MinIO 对象存储，支持文件上传、下载、删除和 presigned URL 生成。实现 `MinioAssetStore(AssetStore)` 适配抽象接口。
...
系统 SHALL 实现 `MinioAssetStore(AssetStore)` 类，匹配 `put()`、`get()`、`delete()` 接口契约，使 `IngestionPipeline` 和 `RetrievalPipeline` 无需修改。
