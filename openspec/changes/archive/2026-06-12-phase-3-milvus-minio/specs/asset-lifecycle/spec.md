...
管理入库过程中图片、视频等资源的完整生命周期：下载→hash 去重→上传 MinIO→创建 Asset 记录→关联知识块。视频链接自动创建 Asset 记录（暂不做语义提取）。
...
- **WHEN** 解析器提取到的图片来自本地文件（DOCX 内嵌图片提取为本地路径）
- **THEN** 系统直接读取文件字节，校验大小和类型

#### Scenario: 图片类型校验

- **WHEN** 获取到图片字节后
- **THEN** 系统校验文件魔数（MIME type）为常见图片格式（PNG/JPEG/GIF/WebP/BMP），非图片则标记 `status=failed`
...
- **WHEN** 图片的 sha256 hash 与已有 Asset 匹配
- **THEN** 系统复用已有 Asset 的 `storage_uri`、`extracted_text` 等字段，仅新增 `source_element_id` 引用，不上传重复文件到 MinIO
...
- **THEN** 系统重新尝试处理（下载→上传），不跳过——之前的失败可能有临时原因
...
系统 SHALL 在解析阶段识别视频链接（HTML `<video>`、Markdown `![video](...)` 等），创建 `Asset(asset_type=video, status=pending)` 记录，阶段 3 不做下载和语义提取。

#### Scenario: 识别视频链接并创建 Asset

- **WHEN** 解析器在文档中识别到视频 URL（YouTube、MP4 链接等）
...
系统 SHALL 限制单文档处理的资源数量和总大小，防止资源爆炸。
