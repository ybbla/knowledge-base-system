...
#### Scenario: 注册 MarkdownParser、DocxParser 和 XlsxParser

- **WHEN** 向注册表注册 `MarkdownParser`（`SUPPORTED_TYPES={"markdown", "md", "txt", "text"}`）、`DocxParser`（`SUPPORTED_TYPES={"docx"}`）和 `XlsxParser`（`SUPPORTED_TYPES={"xlsx"}`）
...
- **AND** `registry.get("MD")` 和 `registry.get("XLSX")` 大小写不敏感返回对应解析器实例
