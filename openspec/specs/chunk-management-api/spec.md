...
通过 `/api/v1/chunks` 提供知识块（KnowledgeChunk）的完整 CRUD、状态治理、索引治理和批量操作能力，面向前端管理台和运维工具。
...
系统 SHALL 通过 `GET /api/v1/chunks` 返回分页知识块列表，并支持按文档、版本、分类、知识类型、业务状态、索引状态、入库任务、关键词、是否有关联资源和是否有来源引用筛选。

#### Scenario: 按文档和索引状态查询知识块
- **GIVEN** 文档 `doc_xxx` 下存在多个索引状态的知识块
- **WHEN** 客户端请求 `GET /api/v1/chunks?doc_id=doc_xxx&index_status=failed&page=1&page_size=20`
- **THEN** 系统仅返回该文档下索引状态为 `failed` 的知识块
- **AND** 响应包含分页 `meta`
...
系统 SHALL 通过 `GET /api/v1/chunks/{chunk_id}` 返回知识块完整内容、所属文档摘要、来源引用、资源引用、索引状态和元数据。
...
- **AND** 响应 SHALL 包含 `index_status`、`indexed_at` 和 `index_error`
...
- **AND** 系统将 `index_status` 设置为 `pending` 或执行重建索引
- **AND** 更新后的内容 SHALL 与索引内容保持一致

#### Scenario: 更新状态时同步索引元数据
- **WHEN** 客户端将知识块状态从 `active` 更新为 `deleted`
- **THEN** 系统更新存储中的 KnowledgeChunk 状态
- **AND** 系统同步向量索引和 BM25 索引中的状态字段

#### Scenario: 禁止内容变化但跳过必要索引
- **WHEN** 客户端修改 `content` 且显式设置 `reindex=false`
- **THEN** 系统返回校验错误或强制将重建索引任务加入队列
...
- **AND** 如果索引缺失或过期，系统 SHALL 将 `index_status` 设置为 `pending` 或触发重建索引

### Requirement: 知识块支持单个和批量重建索引
系统 SHALL 支持通过知识块 API 重建单个或多个知识块的向量索引和 BM25 索引。

#### Scenario: 重建单个知识块索引
- **WHEN** 客户端请求 `POST /api/v1/chunks/{chunk_id}/reindex`
- **THEN** 系统对该知识块重新生成 embedding
- **AND** 系统写入向量索引和 BM25 索引
- **AND** 成功后 `index_status` 为 `indexed`

#### Scenario: 批量重建知识块索引
- **WHEN** 客户端请求 `POST /api/v1/chunks/batch/reindex` 并提供多个 `chunk_ids`
- **THEN** 系统按批处理配置重建索引
- **AND** 对失败知识块记录 `index_error`

### Requirement: 知识块支持批量状态操作
系统 SHALL 支持通过 `POST /api/v1/chunks/batch` 对多个知识块执行状态更新、软删除或恢复。
