...
提供文件上传接口，接收 multipart/form-data 文件，写入当前存储后端，创建 Document 记录并立即触发入库。当 MinIO 启用时写入 `kb-input` Bucket；未启用或不可用时回退本地磁盘。上传时检测内容重复（`source_hash`）和同名文件（按 `title`），支持确认更新参数。

> 新建自 change `align-data-model-and-api-with-updated-design`，日期 2026-06-10；更新自 change `phase-3-milvus-minio`，日期 2026-06-12；更新自 change `fix-root-parse-and-minio-test-isolation`，日期 2026-06-13；更新自 change `simplify-doc-upload-flow`，日期 2026-06-19；更新自 change `simplify-status-model`，日期 2026-06-19。
